...
def train_and_val(config, model, callbacks, mixture_num=None, sub_model_name=None):
    ...
    if is_flat:
        ...
        # Tính steps_per_epoch bằng cách đọc kích thước H5 gốc
        ...
        # Fallback giữ nguyên logic huấn luyện cũ
...
def predict(config, model, mixture_num=None, sub_model_name=None):
    ...
    if is_flat:
        ...
        predictions_file = os.path.join(predictions_dir, f"{sub_model_name}_model")
    ...
    predictions = model.predict(
        ...
        steps=test_steps if (test_size % batch_size == 0 or is_flat) else test_steps + 1,
        verbose=0
    )
...
if __name__ == '__main__':
    ...
    if is_flat:
        ...
        # Fallback to old mixture pipeline
        num_mon_sites = config['num_mon_sites']
        num_mon_inst_test = config['num_mon_inst_test']
        num_mon_inst_train = config['num_mon_inst_train']
        ...
        if not os.path.exists(data_file):
            print("[!] Warning: H5 data file not found at %s." % data_file)
        ...
        for mixture_num, inner_comb in enumerate(mixture):
            ...
            if model_name == 'var-cnn':
                ...
                raise ValueError("Model 'df' is deprecated or not supported.")
